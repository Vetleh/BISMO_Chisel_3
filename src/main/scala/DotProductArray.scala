...
class DotProductArray(val p: DotProductArrayParams) extends Module {
  val io = IO(new Bundle {
    ...
    val a = VecInit.fill(p.m) { Input(Bits(p.dpuParams.pcParams.numInputBits.W)) }
    val b = VecInit.fill(p.n) { Input(Bits(p.dpuParams.pcParams.numInputBits.W)) }
    // DPU outputs from each accumulator
    val out = VecInit.fill(p.m) { VecInit.fill(p.n) { Output(UInt(p.dpuParams.accWidth.W)) } }
  })

  // instantiate the array of DPUs
  val dpu = VecInit.fill(p.m) { VecInit.fill(p.n) {
