...
class DotProductArray(val p: DotProductArrayParams) extends Module {
  val io = IO(new Bundle {
    ...
    val b = Input(Vec(p.n, { Bits(p.dpuParams.pcParams.numInputBits.W) }))

    // DPU outputs from each accumulator
    val out = VecInit.tabulate(p.m) { (x) =>
      VecInit.tabulate(p.n) { (y) =>
        Output(Wire(SInt(p.dpuParams.accWidth.W)))
      }
    }

    // val out = VecInit.tabulate(p.m) { (x) =>
    //   VecInit.tabulate(p.n) { (y) => Output(SInt(p.dpuParams.accWidth.W)) }
    // }
  })

  // instantiate the array of DPUs
  val dpu = VecInit.tabulate(p.m) { (x) =>
    VecInit.tabulate(p.n) { (y) =>
...
//     // val out = VecInit(Seq.fill(p.m) {
//     //   Output(VecInit(Seq.fill(p.n) { Output(SInt(p.dpuParams.accWidth.W)) }))
//     // })

//   })

//   // instantiate the array of DPUs
//   val dpu = Vec(p.m, Vec(p.n, Module(new DotProductUnit(p.dpuParams)).io))
