...
class FetchInterconnect(val myP: FetchStageParams) extends Module {
  val io = IO(new Bundle {
    ...
    val node_out = VecInit.tabulate(myP.getNumNodes()) { (x) =>
  ...
  // val regNodeValid = VecInit(Seq.fill(myP.getNumNodes()) { RegInit(false.B) })
  val regNodeValid = VecInit.tabulate(myP.getNumNodes()) { (x) =>
    RegInit(false.B)
  }
  // the packet data reg does not have init to save LUTs
  val regNodePacket = VecInit.tabulate(myP.getNumNodes()) { (x) =>
    Reg(io.in.bits)
  }
  ...
  for (i <- 0 until myP.getNumNodes()) {
    ...
    io.node_out(i).writeEn := (regNodePacket(i).id === i.U) & regNodeValid(
      i
    )
  }
}
...
//   val lhs_req = VecInit(Seq.fill(myP.numLHSMems) {
//     Output(new OCMRequest(myP.mrp.dataWidth, myP.numAddrBits))
//   })
//   val rhs_req = VecInit(Seq.fill(myP.numRHSMems) {
//     Output(new OCMRequest(myP.mrp.dataWidth, myP.numAddrBits))
//   })
  val lhs_req = VecInit.tabulate(myP.numLHSMems) { (x) =>
    Wire(new OCMRequest(myP.mrp.dataWidth, myP.numAddrBits))
  }
  val rhs_req = VecInit.tabulate(myP.numRHSMems) { (x) =>
    Wire(new OCMRequest(myP.mrp.dataWidth, myP.numAddrBits))
  }
...
class FetchStage(val myP: FetchStageParams) extends Module {
  ...
  routegen.init_new := io.start & !RegNext(false.B, io.start)
